...
class VivaldiVerifierTests(unittest.TestCase):
    def run_verifier(self, available=(), status=0, install_commands=True, build_only=False):
...
'''
            return subprocess.run(
                ...
                env=dict(os.environ, TEST_DIR=directory,
                         ...
                         PACKAGE_OUTPUT="/tmp/vivaldi.pkg.tar.zst" if build_only else "",
    ...
    def test_standalone_build_requires_preinstalled_verifier(self):
        result = self.run_verifier(build_only=True)
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("rpm-tools is required for standalone package builds", result.stderr)
        self.assertNotIn("package request:", result.stdout)

    def test_standalone_build_accepts_existing_verifier(self):
        result = self.run_verifier(available=("rpm", "rpmkeys"), build_only=True)
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertNotIn("package request:", result.stdout)
